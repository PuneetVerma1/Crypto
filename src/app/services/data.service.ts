...
export class DataService {
   ...
   private baseUrl: string = `https://min-api.cryptocompare.com/data/top/mktcapfull?limit=100&tsym=INR&api_key=${this.apiKey}`;
   ...
   getSpecificCoinData(symbol): Observable<BlockItem> {
      return this.http.get(this.priceMultiFullUrl + symbol + '&tsyms=INR&api_key=' + this.apiKey)
         .map(result => {
            let returnedCoin = flattenObject(result["RAW"][symbol]["INR"]);
   ...
   getBetweenDaysPrices(symbol: String, forDays: Number): Observable<any> {
      return this.http.get(this.histoDataUrl + symbol + '&tsym=INR&limit=' + forDays + '&api_key=' + this.apiKey)
         .map(result => {
            return result;
         });
   }
